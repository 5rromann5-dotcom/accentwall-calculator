/* =====================================================================
   PRICING CONFIG
   ЕДИНЫЙ ИСТОЧНИК ЦЕН И ДОПЛАТ ДЛЯ ACCENT WALL CALCULATOR

   ⚠️ ВСЕ ЦЕНЫ, КОЭФФИЦИЕНТЫ И ДОПЛАТЫ МЕНЯЮТСЯ ТОЛЬКО ЗДЕСЬ
   ⚠️ СТРАНИЦЫ НЕ ДОЛЖНЫ СОДЕРЖАТЬ "МАГИЧЕСКИХ ЧИСЕЛ"

   Структура:
   - STEP 2      : базовая цена дизайна
   - STEP 2 VAR  : варианты дизайна (изменение $/sq ft)
   - STEP 3      : подготовка стены
   - STEP 4      : финиш
   - STEP 4.1    : цвета
   ===================================================================== */


/* =====================================================================
   STEP 2 — БАЗОВЫЕ СТАВКИ ДИЗАЙНОВ ($ / sq ft)
   Используются при первом расчёте цены стены
   ===================================================================== */
const DESIGN_BASE_RATE = {
    board_batten: 12,
    modern_geometric: 16,
    slat_shiplap: 12,
    herringbone: 20,
    custom: null          // custom считается вручную
};


/* =====================================================================
   STEP 2 — МИНИМАЛЬНАЯ СТОИМОСТЬ ПРОЕКТА ПО ДИЗАЙНУ ($)
   Защита от слишком маленьких и убыточных проектов
   ===================================================================== */
const DESIGN_MIN_PRICE = {
    board_batten: 900,
    modern_geometric: 1200,
    slat_shiplap: 900,
    herringbone: 1500,
    custom: 1500
};


/* =====================================================================
   STEP 2 — КОЭФФИЦИЕНТ ВЫСОТЫ
   Применяется ТОЛЬКО к базовой цене дизайна
   ===================================================================== */
const HEIGHT_COEF = {
    0: 1.00,   // до 10 ft
    1: 1.25,   // 10–16 ft
    2: 1.50    // выше 16 ft
};


/* =====================================================================
   STEP 2 — ZIP / ЛОКАЦИЯ
   Доплата за логистику (НЕ умножается на коэффициенты)
   ===================================================================== */
const ZIP_POLICY = {
    0: { rate: 0, min: 0 },       // Austin + рядом
    1: { rate: 1, min: 75 },      // ~100 miles
    2: { rate: 2, min: 150 }      // ~300 miles
};


/* =====================================================================
   STEP 2 VARIANT — ВАРИАНТЫ ДИЗАЙНА
   Изменяют СТАВКУ $/sq ft (НЕ коэффициент)
   Первый вариант = базовая ставка (offset = 0)
   ===================================================================== */
const DESIGN_VARIANT_RATE_OFFSET = {
    board_batten:      [0, 1, 2],
    modern_geometric: [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5],
    slat_shiplap:      [0, 0.5, 1],
    herringbone:       [0, 1, 2]
};


/* =====================================================================
   STEP 3 — ПОДГОТОВКА СТЕНЫ ($ / sq ft)
   Добавляется к текущей цене
   ===================================================================== */
const WALL_PREPARATION_RATE = {
    keep_texture: 0,     // Keep Existing Wall Texture
    smooth_wall: 2.5    // Smooth Wall Preparation
};


/* =====================================================================
   STEP 4 — ФИНИШ ($ / sq ft)
   Добавляется к текущей цене
   ===================================================================== */
const FINISH_RATE = {
    one_color: 0,
    multi_color: 0.5,
    partial_color: 0.3,
    solid_wood_paint: 2.0,
    solid_wood_no_paint: 1.5
};


/* =====================================================================
   STEP 4.1 — ЦВЕТА (ФИКСИРОВАННЫЕ ДОПЛАТЫ)
   Добавляется к текущей цене
   ===================================================================== */
const COLOR_SURCHARGE = {
    standard_color: 0,
    customer_custom_color: 50
};


/* =====================================================================
   ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (общие)
   ===================================================================== */

/**
 * Добавляет стоимость, зависящую от площади
 * @param {number} currentPrice — текущая цена
 * @param {number} area — площадь стены (sq ft)
 * @param {number} rate — ставка ($ / sq ft)
 */
function addAreaCost(currentPrice, area, rate) {
    return currentPrice + area * rate;
}

/**
 * Добавляет фиксированную доплату
 * @param {number} currentPrice — текущая цена
 * @param {number} amount — сумма доплаты ($)
 */
function addFixedCost(currentPrice, amount) {
    return currentPrice + amount;
}

/**
 * Применяет ZIP-доплату
 * @param {number} area — площадь стены
 * @param {number} zoneLevel — уровень зоны (0,1,2)
 */
function calculateZipSurcharge(area, zoneLevel) {

    // 🔒 Защита от undefined / NaN
    if (zoneLevel === undefined || zoneLevel === null || isNaN(zoneLevel)) {
        zoneLevel = 0; // считаем как local (Austin)
    }

    const policy = ZIP_POLICY[zoneLevel] || ZIP_POLICY[0];

    const raw = area * policy.rate;
    return Math.max(raw, policy.min);
}

